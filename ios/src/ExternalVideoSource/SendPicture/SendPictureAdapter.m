...
- (void)createtPictureCaptureSession
{
    ...
    if (@available(iOS 10.0, *)) {
        self.sendPicTimer = [NSTimer timerWithTimeInterval:1.0 repeats:YES block:^(NSTimer * _Nonnull timer) {
            if (index % 2 == 0) {
                [self.videoRawdataSender sendVideoFrame:(char *)self.sendBuff1 width:640 height:480 dataLength:640*480*1.5 rotation:ZoomInstantSDKVideoRawDataRotationNone];
            } else {
                [self.videoRawdataSender sendVideoFrame:(char *)self.sendBuff2 width:640 height:480 dataLength:640*480*1.5 rotation:ZoomInstantSDKVideoRawDataRotationNone];
            }
            index++;
        }];
    } else {
