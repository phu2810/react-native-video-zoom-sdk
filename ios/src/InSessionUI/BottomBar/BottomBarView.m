...
- (BOOL)isEqual:(id)object {
    ...
    if (_userId == newObj.userId && [_itemName isEqualToString:newObj.itemName] && [_view isEqual:newObj.view]) {
        return YES;
    }
...
-(NSString *)description {
    return [NSString stringWithFormat:@"View Item Name:%@, userId:%@, view addr:0x%x, is active %@", self.itemName, self.userId, self.view, @(self.isActive)];
}
...
- (void)addThumberViewItem:(ViewItem *)item; {
    ...
    [self removeThumberViewItemWithUserId:item.userId];
